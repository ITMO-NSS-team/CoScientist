...
in abstract - 
Full code with artifacts is available at 
[fork repo we will have just for alembic, with all the benchmark runs, tables, and without coscientist]
Demo video is available at https://youtu.be/NIepDZkMKAA
Interactive web demo is available at [demo video link]
...
- we are always spiltting the main and server env


!! On the the topic of automatic installs of often-messy scientific repos - the ToolMaker approach is to use editable installs on Python 3.12 base image, treating every repo as an editable install, forced to match the current env - this works but does not guarantee that removed/changed behavior won't cause errors. Toolmaker does not have to deal with both mcp packages that are exclusively python 3.10+ and scientific packages 
  - check editable installs and all forced to 3.12 statement on reference ToolMaker reruns
...
To make the Alembic pipeline observable and interactive, we provide a lightweight web dashboard that streams execution in real time and exposes the synthesized MCP server for direct inspection. A user initiates a run by entering a source repository URL, after which the five stages of the pipeline explorer → environment → coder → validator →
  wrapper — unfold live in the browser: a stage rail tracks which agent is currently active and whether it has succeeded or failed, while a collapsible activity feed logs each step in chronological order. 
  
  Instead of raw agent output, the dashboard presents enriched, human-readable views, pass/fail badges summarizing syntax checks and test runs, and end-to-end tool invocations annotated with their inputs, outputs, and, on failure, the underlying error. Beyond passive monitoring, user can manually invoke any tool of the generated server with custom arguments and immediately observe its result, validating the synthesized artifact without leaving the browser.

Figure X. The Alembic web dashboard. The top bar accepts a source repository URL and controls execution; the stage rail beneath it visualizes progress through the explorer → environment → coder → validator → wrapper stages. The left panel shows the exploration report, the generated output files, how to run them (the recorded `setup.sh`), and per-tool
invocation examples; and the right panel shows the generated MCP server together with its validation badges. A collapsible activity feed at the bottom streams individual steps in real time, and completed tools can be invoked interactively with user-supplied arguments.
...
The toolmaker repo and bench had caveats that required fixing for consistent work, but those touched only the insignificant parts of code, 

!! - importing alembic artifacts that ran 3.11 with wplit versions vs the forced 3.12 expected in the benchmark - that's why 

like download command flags, container leakage upon run failiure, importing alembic artefacts (code and setup.sh environment building script) for scoring, and support of models not tracked by cost in litellm, it is available at https://github.com/stas1f1/ToolMaker.
...
!! The tasks split cleanly into two kinds. The clean/one-slip tasks (cytopus, conch, uni, esm,
tabpfn, and the libGL/flexbert set) fix in one shot. The deep-cascade tasks (flowmap, musk) expose a chain of py3.11→py3.12 dependency-resolution
conflicts — exactly the failure mode you predicted from the native-version mismatch. Each fix reveals the next layer, because Alembic transcribed a
dependency set that only resolved consistently under the repo's original Python.
...
4.1 Case study - onetask where we have good pictures / visualizations / interesting data sturcture to work with - analyze the nature of the task and repo, its related paper, what tools were generated ands how successful, highlight one or few tools' usage - what went in and what we got out (must be interesting data, transformations, picture etc) 
