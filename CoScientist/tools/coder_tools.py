...
def run_sandbox_task_tool(
    ...
    """
    Submits a task to the OpenHands Sandbox server. 
    Use it for heavy tasks or tasks that require long execution time.

    Args:
        task: Detailed instructions/prompt for the AI agent inside the sandbox.
        dataset_url: Optional direct URL to a ZIP archive to extract into the workspace (only .zip).
        poll_interval: Time (in seconds) to wait between status checks.
        wait: Whether to queue the task and wait if the sandbox is busy.

    Returns:
        A dictionary containing the final execution status and result summary.
    """
    sandbox_url = settings.web.sandbox_url
    api_url = f"{sandbox_url.rstrip('/')}/api/v1"

    try:
        if dataset_url:
            response = httpx.post(
                f"{api_url}/run-external",
                json={"task": task, "dataset_url": dataset_url, "wait": wait},
                timeout=30.0
            )
        else:
            response = httpx.post(
                f"{api_url}/run",
                json={"task": task, "wait": wait},
                timeout=30.0
            )

        if response.status_code == 429:
            print("[-] Sandbox is busy. Another task is currently running.", file=sys.stderr)
            return {
                "status": "error",
                "error": f"Sandbox busy: {response.json().get('detail', 'Please try again later')}"
            }
        ...
        return {"status": "error", "error": f"Submission failed: {e}"}

    task_id = data["task_id"]
    
    # Construct or retrieve the Web UI watch URL
    watch_url = data.get("watch_url")
    if not watch_url:
        watch_url = f"{sandbox_url.rstrip('/')}/?task_id={task_id}"

    # Step 2: PROMINENTLY print the monitoring URL so the human developer sees it instantly
    print("=" * 80)
    print(f"[+] Task successfully submitted! ID: {task_id}")
    print(f"[+] WATCH LIVE IN WEB CONSOLE:")
    print(f"    👉 {watch_url} 👈")
    print("=" * 80)
    sys.stdout.flush()

    # Step 3: Block and poll the status endpoint until the task completes
    print("[*] Waiting for execution to complete...", end="", flush=True)
    
    completed_statuses = {"completed", "cooldown", "error", "cancelled"}
    last_status = "queued"
    ...
    while True:
        try:
            ...
            task_details = status_data.get("current_task") or {}
            status = task_details.get("status", "unknown")

            if status != last_status:
                print(f"\n[*] Status changed: {last_status} -> {status}", end="", flush=True)
                last_status = status
            else:
            ...
            if status in completed_statuses:
                ...
                return {
                    ...
                    "error": task_details.get("error", None),
                    "watch_url": watch_url
                }

        except Exception as e:
            # We don't exit immediately on transient network errors
            print(f"\n[!] Warning: Failed to fetch status ({e}). Retrying...", end="", flush=True)

        time.sleep(poll_interval)

if __name__ == "__main__":
    # Example usage / testing
    test_task = "Write a python script that prints 'Hello from Sandbox!' and creates a file named hello.txt containing the current time."
    result = run_sandbox_task_tool(task=test_task)
