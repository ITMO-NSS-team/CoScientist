...
|---|--------|:------:|----------------|:------:|
| **N1** | **Set `temperature=0` (+ small `top_p`) on all agents; report pass@k over k≥3 seeds, not a single number** | ~1h | Collapses the run-to-run variance that is both your stability pain and the paper's reproducibility risk. Confirmed unset today. | [02](./02-stability.md), [03](./03-benchmarking.md) |
...
| **N4** | **Explorer reads the repo's own `tests/`+`examples/`** (it is currently told to *skip* them) | ~1h | Those files are the single best source of real signatures (F1), real fixture paths (F21) and real sample sizes (F18) — three open problems fed by one cheap instruction change. | [02](./02-stability.md) |
