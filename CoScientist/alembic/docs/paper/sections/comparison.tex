...
More broadly, LATM \citep{latm2024}, TroVE \citep{trove2024}, and ReGAL \citep{regal2024} also have LLMs write reusable tools, but they grow or refactor a Python function library \emph{across many solved tasks} using execution traces as feedback; Alembic instead performs a single, no-history synthesis pass over one repository and its output is a served MCP endpoint rather than an in-context callable. On the delivery side, a growing set of OpenAPI-to-MCP generators (e.g., Speakeasy, \texttt{mcp-generator-cli}) convert already-typed REST specs into MCP servers — a fundamentally easier input than the undocumented, dependency-heavy research code Alembic targets.
