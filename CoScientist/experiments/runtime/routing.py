...
def task_coverage_blob(state: Mapping[str, Any], task: ExperimentTask) -> str:
    ...
    ref = str(getattr(task.design, "operation_ref", "") or "").strip().upper()
    ops_index = _ops_index_from_state(state)
    if ref and ref in ops_index:
        statement = str(ops_index[ref].get("statement") or "")
        if statement.strip():
            return statement
    return " ".join((
        str(task.name or ""),
        str(task.description or ""),
        str(task.rationale or ""),
        str(task.design.experiment_question or ""),
        " ".join(str(art.path_or_tool or "") for art in task.design.analysis_artifacts),
    ))


def inventory_covers_task(state: Mapping[str, Any], task: ExperimentTask) -> bool:
    """This-run retrieve covers the task: named, slot fit, or an already-bound tool."""
    ...
    if match_named_inventory_tool(blob, by_tool) is not None:
        return True
    if match_slot_inventory_tool(blob, by_tool) is not None:
        return True
...
def match_session_inventory_tool(
    ...
    """Named or already-bound tool from session inventory (rewrite target)."""
    from CoScientist.experiments.capabilities.inventory import (
        index_inventory_tools,
        match_named_inventory_tool,
    )
