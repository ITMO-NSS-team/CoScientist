...
def assess_experiment_inventory_feasibility(callback_context: Any) -> None:
    ...
    ask = str(state.get("experiment_source_request") or "").strip()
    needed = request_capabilities(ask) if ask else []
...
def skip_when_experiment_not_feasible(callback_context: Any) -> Optional[types.Content]:
    ...
    state = callback_context.state
    # ADK State не наследует Mapping (MRO: State -> object), поэтому
    # isinstance(state, Mapping) здесь всегда False и гард молчал.
...
def skip_when_experiment_stage_complete(callback_context: Any) -> Optional[types.Content]:
    ...
    state = callback_context.state
    # См. выше: утиная проверка вместо isinstance(state, Mapping), которая на
    # ADK State никогда не проходит и делала весь гейт недостижимым.
    getter = getattr(state, "get", None)
    if not callable(getter):
        return None
    runtime = getter("experiment_runtime")
    if not isinstance(runtime, dict):
        # Открытый гейт на завершённой стадии — это повторное планирование
        # целого эксперимента, поэтому говорим, что он увидел. runtime=NoneType
        # означает, что завершение не доехало до вызывающего через границу
        # AgentTool (см. _REVIEW_OWNED_STATE_KEYS в experiments/review.py).
        audit(
            logger,
