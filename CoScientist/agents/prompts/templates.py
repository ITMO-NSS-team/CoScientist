...
def orchestrator(ctx: PromptContext) -> str:
    ...
    if has_research_graph:
        ...
        research_graph_section = (
            ...
            "bases), then delegate.\n"
            "- To verify EACH hypothesis: call `research_set_focus(<HYPOTHESIS id>)` "
            "FIRST (always the hypothesis id — never a tool/method id), THEN delegate "
            "evidence-gathering, defaulting to ResearchAgent for LITERATURE evidence "
            "(only use Coder/TaskExecutor when a concrete computation is needed and a "
            "tool exists). Every piece of evidence the worker records is then "
            "auto-attached to that hypothesis, moving it to under_verification and "
            "letting the background validator judge it. Do NOT skip set_focus, do NOT "
            "set the verdict yourself, and do the SAME for every hypothesis so none "
            "is left un-investigated.\n"
