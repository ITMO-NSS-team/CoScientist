"""Cross-run knowledge graph — the system's persistent *graph memory*.

While ``KnowledgeGraph`` (memory.py) records ONE session's execution trace, this
store accumulates the *semantic* layer ACROSS runs: the typed domain entities and
relations extracted from agent outputs (semantic.py). It is what lets a later run
reuse what earlier runs established.
...
# Process-wide persistent memory (cross-run).
