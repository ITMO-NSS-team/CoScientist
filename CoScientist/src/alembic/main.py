...
async def run_agent(
    ...
    artifact_guard_path: str | None = None,
) -> str:
    """Run an agent, retrying if guards (write_report / expected artifact) are not satisfied."""
    ...
    for attempt in range(MAX_GUARD_RETRIES + 1):
        ...
        if artifact_guard_path and not Path(artifact_guard_path).exists():
            nudges.append(
                f"A required artifact is missing: {artifact_guard_path}. "
                "For the environment stage you MUST write Dockerfile, run build_docker_image "
                "successfully (so .docker_image exists), then write_report before finishing."
            )
...
STAGES = ("explorer", "coder", "environment", "validator")
...
async def run_pipeline(repo_url: str, resume_from: str | None = None):
    ...
    base = WORKDIR / name
    docker_image_marker = str((base / "repos" / ".docker_image").resolve())

    for sid in (f"{name}_explorer", f"{name}_coder",
                f"{name}_environment", f"{name}_validator"):
    ...
    # ── Stage 3: Environment (Dockerfile + image after server/tests exist) ──
    if _should_run("environment"):
        _banner(3, f"Environment ({repo_url})")
        await run_agent(
            environment_agent, session_service, f"{name}_environment", repo_url,
            required_report="environment", artifact_guard_path=docker_image_marker,
        )
