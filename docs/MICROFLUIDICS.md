...
### Опросник оператора (порт опросника VibePAV)

К документу ТЗ в ревью автоматически добавляется раздел **«Уточняющие вопросы
оператору»**: по одному вопросу (с подсказкой-примером из VibePAV) на каждый
блок, где остались поля «не задано», с перечислением этих полей
(`microfluidics/questionnaire.py`). Ответы пишутся в поле правок:

- `Q1: <ответ>` — поле заполняется со статусом «уточнено оператором»;
- `Q2: не знаю` (или пропуск вопроса) — поле остаётся «не задано», система
  продолжает работу без него;
- `Q3: на усмотрение агента` — агент подставляет рабочее значение из
  отраслевого контекста;
- **Accept** без ответов — продолжить со всеми незакрытыми вопросами.

Отвечать можно на любое подмножество вопросов; после Revise агент переписывает
ТЗ и опросник пересобирается по оставшимся пробелам.
